...
func init() {
	...
	busCmd.Flags().String("mta-api-key", "", "API key used for calls to the MTA API")
	err := busCmd.MarkFlagRequired("mta-api-key")
	if err != nil {
		os.Exit(1)
	}

	busCmd.Flags().String("stop-id", "", "Stop ID for the station")
